...
export class PanZoomService {
  ...
  focusSelectedNode(
    ...
    const selectedNode = this._findNode(nodeId, elementRef);
  ...
  handleRoadmapScrollBounds(
    ...
    if (lowerNodeRect.bottom < this._transformMargin) {
    ...
    if (topNodeRect.top > window.innerHeight - this._transformMargin) {
